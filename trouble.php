...
<div class="accordion">
    <?php foreach ($classMapping as $classId => $className): ?>
        <div class="collapse collapse-arrow bg-base-200 shadow-xl mb-4">
            <input type="radio" name="accordion" id="accordion-<?= $classId ?>" class="peer" />
            <label for="accordion-<?= $classId ?>" class="collapse-title text-xl font-bold peer-checked:bg-secondary peer-checked:text-black relative">
                <span class="absolute left-1/2 transform -translate-x-1/2"><?= $className ?></span>
            </label>
                            ...
                            if (!empty($lotions)):
                                ...
                                <li class="spacing">
                                    ...
                                    <p class="description-text">
                                        <?= nl2br(str_replace('、', '、<br>', htmlspecialchars($component['description'], ENT_QUOTES, 'UTF-8'))) ?>
                                    </p>
                                    <p class="between-spacing section-title text-lg font-bold my-4">おすすめの化粧水は</p>

                                    <ul class="mt-2 flex flex-wrap justify-center gap-4">
                                        <?php foreach ($lotions as $lotion): ?>
                                            <li class="spacing w-full sm:w-64">
                                                <div class="card bg-base-100 shadow-xl w-full h-full">
                                                    <figure class="h-48 w-full flex items-center justify-center">
                                                        <img class="object-contain w-full h-full" src="data:image/jpeg;base64,<?= base64_encode($lotion['image']) ?>" alt="<?= htmlspecialchars($lotion['name'], ENT_QUOTES, 'UTF-8') ?>">
                                                    </figure>
                                                    <div class="card-body">
                                                        <h2 class="card-title" style="font-size: 0.975rem;"><?= htmlspecialchars($lotion['name'], ENT_QUOTES, 'UTF-8') ?></h2>
